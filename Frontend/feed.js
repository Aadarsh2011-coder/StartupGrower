...
const SPREADSHEET_ID = "1525cOECoV2sXJjpgrjicGp6MIREXuxtCRp9Be27MK6w";
const API_KEY = "${GOOGLE_SHEETS_API_KEY}";
const RANGE = "Form responses 1!A:M";
...
function renderFeed(data) {
  ...
  data.forEach(item => {
    ...
    card.innerHTML = `
      ...
      ${item.tagline ? `<p class="tagline"><strong>💡 Tagline:</strong> ${escapeHtml(item.tagline)}</p>` : ""}
      ${item.description ? `<p><strong>📝 Description:</strong> ${escapeHtml(item.description)}</p>` : ""}
      ${item.tags ? `<div class="tags"><strong>🏷️ Tags:</strong>${item.tags.split(",").map(t => `<span class="tag">${escapeHtml(t.trim())}</span>`).join("")}</div>` : ""}
    ...
    upvoteBtn.addEventListener("click", async () => {
      ...
      try {
        const res = await fetch(`${WEB_APP_URL}?row=${item.rowNumber}`, { method: 'GET' });
...
// Initialize
document.addEventListener('DOMContentLoaded', function() {
  console.log("🚀 StartupGrower Feed System Initialized (Most Upvoted first)");
  fetchFeedData();
});
