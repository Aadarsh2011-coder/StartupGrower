...
let selectedGateway = 'razorpay'; // 'razorpay' or 'fastspring'
let selectedCurrency = 'INR';

// ===== FASTSPRING CONFIGURATION =====
const FASTSPRING_CONFIG = {
  storefront: 'startupgrower.fastspring.com', // REPLACE WITH YOUR STORE URL
  productMap: {
    'starter': 'startupgrower-starter-plan',
    'growth': 'startupgrower-growth-plan',
    'pro': 'startupgrower-pro-plan',
    'elite': 'startupgrower-elite-plan',
    'ultimate': 'startupgrower-ultimate-plan'
  }
};
...
function setupCurrencySelector() {
  ...
  options.forEach(option => {
    option.addEventListener('click', () => {
      ...
      console.log(`💰 Gateway selected: ${selectedGateway}, Currency: ${selectedCurrency}`);
...
function updatePriceDisplay() {
  ...
  const displayCurrency = selectedCurrency === 'INR' ? 'inr' : 'usd';
  
  allPrices.forEach(price => {
    if (price.dataset.currency === displayCurrency) {
      price.classList.add('active');
    } else {
      price.classList.remove('active');
    }
...
async function processPayment(plan, formData) {
  return new Promise((resolve, reject) => {
    if (plan === 'free') {
      resolve({ success: true, paymentId: 'free_plan', gateway: 'free' });
      return;
    }

    console.log(`🔄 Processing payment via ${selectedGateway} for ${selectedCurrency}`);

    if (selectedGateway === 'razorpay') {
      // ===== RAZORPAY FLOW (INR only) =====
      createRazorpayOrder(plan, formData).then(orderData => {
        if (typeof Razorpay === 'undefined') {
          reject(new Error('Razorpay gateway not loaded. Please refresh the page.'));
          return;
        }
      ...
      // ===== FASTSPRING FLOW (International) =====
      launchFastSpringCheckout(plan, formData, resolve, reject);
    }
  });
}

// FastSpring checkout launcher
function launchFastSpringCheckout(plan, formData, resolve, reject) {
  // Map your plans to FastSpring product IDs
  const productId = FASTSPRING_CONFIG.productMap[plan];
  
  if (!productId) {
    reject(new Error(`No FastSpring product configured for ${plan} plan. Please contact support.`));
    return;
  }
  
  console.log(`🚀 Launching FastSpring for product: ${productId}`);
  ...
  if (typeof FastSpring === 'undefined') {
    ...
    script.dataset.storefront = FASTSPRING_CONFIG.storefront;
    script.dataset.continuous = 'true';
    
    script.onload = () => openFastSpringPopup(productId, formData, resolve, reject);
    script.onerror = () => reject(new Error('Failed to load FastSpring payment gateway'));
    
    document.head.appendChild(script);
  } else {
    openFastSpringPopup(productId, formData, resolve, reject);
  }
}

function openFastSpringPopup(productId, formData, resolve, reject) {
  try {
    FastSpring.Builder.push({
      product: productId,
      data: {
        customerEmail: formData.email,
        customerName: formData.founders,
        productName: formData.toolName,
        website: formData.url,
        plan: selectedPlan,
        submissionDate: new Date().toISOString()
      },
      tags: [selectedPlan, 'startupgrower', 'international'],
      callback: function(data) {
        console.log('FastSpring callback:', data);
        
        if (data && data.order && data.order.reference) {
          // Success
          resolve({
            success: true,
            paymentId: data.order.reference,
            gateway: 'fastspring',
            orderData: data
          });
        } else {
      ...
      error: function(error) {
        ...
        reject(new Error('FastSpring payment error: ' + (error.message || 'Unknown error')));
      }
    });
  } catch (error) {
    console.error('Error launching FastSpring:', error);
    reject(new Error('Failed to launch payment gateway'));
  }
}
...
function showPricingSection() {
  ...
  if (currencySection) {
    console.log("💱 Showing currency selector...");
...
function setupFormSubmission() {
  ...
  form.addEventListener("submit", async (e) => {
    ...
    try {
      ...
      // ===== STEP 2: PROCESS PAYMENT =====
      let paymentResult = { success: true, paymentId: 'free_plan', gateway: 'free' };
      
      if (selectedPlan !== 'free') {
        showToast(`💳 Processing ${PRICING_PLANS[selectedPlan].name} payment via ${selectedGateway}...`, 'info');
        ...
        showToast(`✅ Payment successful via ${paymentResult.gateway}! Submitting your tool...`, "success");
        console.log('💰 Payment completed:', paymentResult);
      }

      // Add payment info to form data
      formData.paymentId = paymentResult.paymentId;
      formData.paymentGateway = paymentResult.gateway;
      ...
      if (result.success) {
        ...
        const successMessage = selectedPlan === 'free' 
          ? "🎉 Your tool has been submitted successfully! It will appear in the feed shortly." 
          : `🎉 Your tool has been submitted successfully! Payment ID: ${paymentResult.paymentId} (via ${paymentResult.gateway})`;
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  console.log("🔐 Using Apps Script URL:", CONFIG.APPS_SCRIPT_URL);
  console.log("🖼️ imgBB Integration Active");
  console.log("💳 Payment Gateways: Razorpay (INR) & FastSpring (International)");
